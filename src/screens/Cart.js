...
export default function Cart() {
  ...
  const handleCheckOut = async () => {
    ...
    let response = await fetch("https://tomatomernappbackend.onrender.com/api/orderData", {
      method: 'POST',
      headers: {
    ...
    <div>
      <div className='container m-auto mt-5 table-responsive table-responsive-sm table-responsive-md'>
        <table className='table table-hover'>
          <thead className='text-light bg-dark fs-4'>
            <tr>
              <th scope='col'>#</th>
          ...
          <tbody>
            {data.map((food, index) => (
              <tr className={index % 2 === 0 ? 'bg-light text-dark' : 'bg-secondary text-light'}>
